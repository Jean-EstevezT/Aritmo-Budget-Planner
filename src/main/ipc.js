...
function registerIpcHandlers() {
  ...
  ipcMain.handle(CHANNELS.GET_EXPENSES, async () => {
  ...
  ipcMain.handle(CHANNELS.ADD_EXPENSE, async (_, data) => {
    const payload = {
      date: data.date,
  ...
  ipcMain.handle(CHANNELS.GET_EXPENSE_CATEGORIES, async () => knex('expense_categories').select('*').orderBy('name'));
  ipcMain.handle(CHANNELS.ADD_EXPENSE_CATEGORY, async (_, name) => knex('expense_categories').insert({ name }));
  ipcMain.handle(CHANNELS.DELETE_EXPENSE_CATEGORY, async (_, id) => {
    const cid = toIntOrNull(id);
    await knex('expenses').where({ category_id: cid }).del();
    return knex('expense_categories').where({ id: cid }).del();
  });

  ipcMain.handle(CHANNELS.GET_INCOME, async () => {
  ...
  ipcMain.handle(CHANNELS.ADD_INCOME, async (_, data) => {
    const payload = {
      date: data.date,
      source: data.source,
      amount: toNumber(data.amount),
  ...
  ipcMain.handle(CHANNELS.GET_INCOME_CATEGORIES, async () => knex('income_categories').select('*').orderBy('name'));
  ipcMain.handle(CHANNELS.ADD_INCOME_CATEGORY, async (_, name) => knex('income_categories').insert({ name }));
  ipcMain.handle(CHANNELS.DELETE_INCOME_CATEGORY, async (_, id) => {
    const cid = toIntOrNull(id);
  ...
  ipcMain.handle(CHANNELS.DELETE_TRANSACTION, async (_, { type, id }) => {
    try {
  ...
  ipcMain.handle(CHANNELS.GET_TRANSACTION, async (_, { type, id }) => {
    const table = type === 'expenses' ? 'expenses' : 'income';
    const row = await knex(table).where({ id: toIntOrNull(id) }).first();
    return coerceFields(row, ['amount']);
  });

  ipcMain.handle(CHANNELS.UPDATE_TRANSACTION, async (_, { type, id, ...data }) => {
    const table = type === 'expenses' ? 'expenses' : 'income';
    const payload = {
      date: data.date,
      amount: toNumber(data.amount),
  ...
  ipcMain.handle(CHANNELS.GET_CATEGORY, async (_, { type, id }) => {
    const table = type === 'expense' ? 'expense_categories' : 'income_categories';
    return knex(table).where({ id: toIntOrNull(id) }).first();
  });

  ipcMain.handle(CHANNELS.UPDATE_CATEGORY, async (_, { type, id, name }) => {
    const table = type === 'expense' ? 'expense_categories' : 'income_categories';
    return knex(table).where({ id: toIntOrNull(id) }).update({ name });
  });

  ipcMain.handle(CHANNELS.GET_EXPENSE_BUDGET_DATA, async () => {
    try {
  ...
  ipcMain.handle(CHANNELS.GET_INCOME_BUDGET_DATA, async () => {
    try {
  ...
  ipcMain.handle(CHANNELS.CLEAR_DATABASE, async () => {
